...
class CNNWithMaxPool(NN):
    def __init__(self, input_shape, num_classes, save=False, device="cpu"):
        ...
        self.flatten()
        self.linear(
            in_features=8 * (input_shape[1] // 2) * (input_shape[2] // 2),
            out_features=num_classes,
        )


def build_cnn_maxpool(input_shape=(3, 8, 8), num_classes=5):
    model = CNNWithMaxPool(input_shape, num_classes).to(DEVICE)
    model.eval()
    x = torch.rand(input_shape)
    return model, x
...
class TestFullMatrix(unittest.TestCase):
    def test_km_invariant(self):
        for name, builder in [("CNN", build_cnn), ("MLP", build_mlp), ("BN", build_cnn_bn), ("MaxPool", build_cnn_maxpool)]:
            model, x = builder()
            gmc = GradientMatrixComputer(model)
            mat = gmc.forward(x)
